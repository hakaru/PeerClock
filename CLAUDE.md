...
- アプリターゲット追加時に `NSLocalNetworkUsageDescription` と `NSBonjourServices: ["_peerclock._tcp", "_peerclock._udp"]` が Info.plist に必要（Discovery は TCP リスナーだが Configuration デフォルトの serviceType は `_peerclock._udp`）
